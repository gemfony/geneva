...
void producer(
    ...
    while(cycleCounter < nProductionCycles) {
        ...
        if(putTimeout.count() > 0.) {
            ...
            if(putTimeouts > highestPutTimeouts)
                highestPutTimeouts = putTimeouts;
    ...
    while(nReceived < nProductionCycles) {
        if(getTimeout.count() > 0.) {
            ...
            if(getTimeouts > highestGetTimeouts)
                highestGetTimeouts = getTimeouts;
    ...
    { // Output the results
        ...
        if(putTimeouts > 0 || getTimeouts > 0) {
            std::cout << " with " << totalPutTimeouts << " put time-outs (max "
                      << highestPutTimeouts << ") and " << totalGetTimeouts
                      << " get time-outs (max " << highestGetTimeouts << ")";
        }
        std::cout << "." << std::endl;
    }
}
