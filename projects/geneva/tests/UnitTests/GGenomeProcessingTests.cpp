...
/******************************************************************************/
/**
 * Pins the entity/genome SPLIT of GGenome's member list, not merely the round trip.
 *
 * GGenome::save() emits entityMembers_() unconditionally and BEFORE the `genome_omitted`
 * marker, then genomeMembers_() after it, so a results-only return drops the input
 * parameters while the computed results still travel. Which group a member belongs to is a
 * convention the compiler cannot check: a member placed in the wrong group still
 * round-trips (save() and load() read the same two functions), so the full-round-trip case
 * above stays green while that member silently vanishes on this one path.
 *
 * This case exercises exactly that path.
 */
TEST_CASE("GGenome: a results-only return keeps the entity half and drops the genome half", "[candidate][wire][regression]") {
    NewSphere ind(5, 2, 3);
    ind.setAssignedIteration(4711); // entity-half state that must survive
    ind.process();                  // results + PROCESSED status: also entity half
    REQUIRE(ind.is_processed());

    std::vector<double> sent_values;
    ind.streamline<double>(sent_values);
    ...
    std::string wire;
    {
        Gem::Courtier::GWireSerializationScope const scope(&ctx);
        wire = ind.toString(Gem::Common::serializationMode::GEM_BINARY);
    }

    NewSphere received;
    {
        Gem::Courtier::GWireSerializationScope const scope(&ctx);
        received.fromString(wire, Gem::Common::serializationMode::GEM_BINARY);
    }

    // The entity half survived. If any of these members were moved into genomeMembers_(),
    // it would be dropped here and this check would fail -- while every other test passed.
    CHECK(received.is_processed());
    CHECK(received.raw_fitness(0) == Approx(ind.raw_fitness(0)));
    CHECK(received.getAssignedIteration() == 4711);

    // The genome half was dropped -- that is the whole point of a results-only return.
    std::vector<double> received_values;
    received.streamline<double>(received_values);
    CHECK(received_values.empty());

    // ...and the payload really is smaller than a full return would be.
    std::string full_wire;
    {
        Gem::Courtier::GWireSerializationContext full_ctx = ctx;
        full_ctx.returning = false;
        Gem::Courtier::GWireSerializationScope const scope(&full_ctx);
        full_wire = ind.toString(Gem::Common::serializationMode::GEM_BINARY);
    }
    CHECK(wire.size() < full_wire.size());
}
