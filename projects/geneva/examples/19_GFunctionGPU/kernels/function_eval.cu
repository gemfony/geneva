// CUDA evaluation kernel for the GFunctionIndividual GPU demo (example 19).
//
// This file is NOT compiled at build time: the courtier GPU consumer's CUDA backend reads it at run time
// (its path comes from config/GGPUConsumer.json) and compiles it with NVRTC. It must follow the
// framework's evaluation ABI: an extern "C" (un-mangled) entry taking the flat row-major parameter
// buffer, the item count and dimension, an opaque problem-constant blob, and writing one fitness per item.
//
// Problem constant: a single int "function id" matching Gem::Geneva::Benchmarks::FUNC_* (0..14). The math
// below MIRRORS geneva/individuals/GBenchmarkFunctions.hpp -- the SAME shared CPU reference that
// GFunctionIndividual::evaluate() runs -- so a CPU run (--consumer stc) and a GPU run (--consumer gpu)
// produce the same fitness. The demo's parity-check mode verifies this. (Identical to the GCUDAOptBenchmark
// kernel; NVRTC has no include path to the Geneva headers, so the math is spelled out standalone. Keep the
// three in sync if the benchmark functions ever change.)
//
