	...
	: public GOptimizationAlgorithmT2<executor_type>
{
	 ...
	 void serialize(Archive & ar, const unsigned int) {
		 ...
		 & BOOST_SERIALIZATION_NVP(m_amalgamation_likelihood);
	 }
	 /////////////////////////////////////////////////////////////////////////////
...
public:
	 ...
	 virtual void compare(
		 ...
		 compare_t(IDENTITY(m_amalgamation_likelihood, p_load->m_amalgamation_likelihood), token);
	 ...
	 void setAmalgamationLikelihood(double amalgamationLikelihood) {
		 if (amalgamationLikelihood < 0. || amalgamationLikelihood > 1.) {
			 glogger
				 << "In GBaseParChildT2::setAmalgamationLikelihood(" << amalgamationLikelihood << "): Error!" << std::endl
				 << "Received invalid likelihood for amalgamation. Must be in the range [0:1]." << std::endl
				 << GEXCEPTION;
		 }

		 m_amalgamation_likelihood = amalgamationLikelihood;
	 }
  ...
  */
	 double getAmalgamationLikelihood() const {
		 return m_amalgamation_likelihood;
	 }
...
protected:
	 ...
	 virtual void doRecombine() {
		 ...
		 std::bernoulli_distribution amalgamationWanted(m_amalgamation_likelihood); // true with a likelihood of m_amalgamation_likelihood
		 for(auto item: boost::make_iterator_range(GOptimizationAlgorithmT2<executor_type>::data.begin()+m_n_parents, GOptimizationAlgorithmT2<executor_type>::data.end())) {
			 ...
			 if (m_n_parents > 1 && amalgamationWanted(GOptimizationAlgorithmT2<executor_type>::m_gr)) { // Create individuals using a cross-over scheme
				 std::shared_ptr<GParameterSet> bestParent = this->front();
				 std::shared_ptr<GParameterSet> combiner = (m_n_parents > 2) ? (*(this->begin() + this->m_uniform_int_distribution(
				 ...
				 item->GObject::load(bestParent->amalgamate(combiner));
	 ...
	 void actOnStalls() {
		 ...
		 if (this->getNParents() > 1) {
			 for(auto item: boost::make_iterator_range(this->begin(), this->begin() + this->getNParents())) {
				 item->updateAdaptorsOnStall(this->getStallCounter());
			 }
		 }
	 }
	 ...
	 void randomRecombine(std::shared_ptr<executor_type>& child) {
		 ...
		 if(m_n_parents==1) {
			 ...
			 parent_pos = this->m_uniform_int_distribution(
				 GOptimizationAlgorithmT2<executor_type>::m_gr
	 ...
	 void sortMunu1pretainMode() {
		 ...
		 if(m_n_parents==1 || GOptimizationAlgorithmT2<executor_type>::inFirstIteration()) { // Falls back to MUPLUSNU_SINGLEEVAL mode
			 ...
			 double bestTransformedChildFitness_MinOnly  = (*(GOptimizationAlgorithmT2<executor_type>::data.begin() + m_n_parents))->minOnly_fitness();
			 double bestTransformedParentFitness_MinOnly = (*(GOptimizationAlgorithmT2<executor_type>::data.begin()))->minOnly_fitness();

			 // Leave the best parent in place, if no better child was found
			 if(bestTransformedChildFitness_MinOnly < bestTransformedParentFitness_MinOnly) { // A better child was found. Overwrite all parents
				 std::swap_ranges(
					 GOptimizationAlgorithmT2<executor_type>::data.begin()
					 ,GOptimizationAlgorithmT2<executor_type>::data.begin()+m_n_parents
					 ,GOptimizationAlgorithmT2<executor_type>::data.begin()+m_n_parents
	 ...
	 double m_amalgamation_likelihood = DEFAULTAMALGAMATIONLIKELIHOOD; ///< Likelihood for children to be created by cross-over rather than "just" duplication (note that they may nevertheless be mutated)
