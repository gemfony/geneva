...
public:
	...
	/** @brief A constructor for vectors */
	G_API_COMMON GFileParsableI(
		const std::vector<std::string> &, const std::vector<std::string> &, const bool &
	);
...
public:
	...
	), def_val_(def_val), par_(def_val) { /* nothing */ }
...
public:
	 ...
	 */
	GFileSingleParsableParameterT(
		const std::string &optionNameVar, const parameter_type &def_val
	)
		: GSingleParmT<parameter_type>(
...
protected:
	...
	virtual void load(const boost::property_tree::ptree &pt) {
		GSingleParmT<parameter_type>::par_ = pt.get((GParsableI::optionName(0) + ".value").c_str(),
...
public:
	...
	GFileReferenceParsableParameterT(
		...
		: GSingleParmT<parameter_type>(
		optionNameVar, commentVar, isEssentialVar, def_val
	), storedReference_(storedReference) { /* nothing */ }
...
public:
	...
	GCombinedParT(
		...
		isEssentialVar
	), par0_(def_val0), def_val0_(def_val0), par1_(def_val1), def_val1_(def_val1),
...
protected:
	...
	virtual void save(boost::property_tree::ptree &pt) const {
		...
		for (it = GVectorParT<parameter_type>::def_val_.begin();
			  it != GVectorParT<parameter_type>::def_val_.end(); ++it) {
...
protected:
	...
	virtual void load(const boost::property_tree::ptree &pt) {
		...
		for (std::size_t i = 0; i < GArrayParT<parameter_type, N>::par_.size(); i++) {
			GArrayParT<parameter_type, N>::par_.at(i) = pt.get(
				(GParsableI::optionName(0) + "." + boost::lexical_cast<std::string>(i) + ".value").c_str(),
				GArrayParT<parameter_type, N>::def_val_.at(i));
		}
	}
	...
	virtual void save(boost::property_tree::ptree &pt) const {
		...
		for (std::size_t i = 0; i < GArrayParT<parameter_type, N>::def_val_.size(); i++) {
			...
			pt.add((GParsableI::optionName(0) + "." + boost::lexical_cast<std::string>(i) + ".value").c_str(),
					 GArrayParT<parameter_type, N>::par_.at(i));
		}
	}
...
public:
	...
	), storedReference_(storedReference), def_val_(defVal), implicitAllowed_(implicitAllowed),
...
protected:
	...
	virtual void save(boost::program_options::options_description &desc) const {
		...
		if (GCL_IMPLICIT_ALLOWED == implicitAllowed_) {
			...
			desc.add_options()(
				(this->optionName()).c_str(), po::value<parameter_type>(&storedReference_)->default_value(def_val_),
				(this->comment()).c_str()
			);
		}
	}
	...
	virtual std::string content() const {
		...
		result << this->optionName() << " :\t" << storedReference_ << "\t" <<
		((storedReference_ != def_val_) ? "default: " + boost::lexical_cast<std::string>(def_val_) : std::string(""));
		return result.str();
	}
...
public:
	...
	std::shared_ptr <fileParsableDerivative> file_at(const std::string &optionName) {
		...
		it;
		if ((it = std::find_if(file_parameter_proxies_.begin(), file_parameter_proxies_.end(),
									  findFileProxyByName(optionName))) == file_parameter_proxies_.end()) {
	...
	std::shared_ptr <clParsableDerivative> cl_at(const std::string &optionName) {
		...
		it;
		if ((it = std::find_if(cl_parameter_proxies_.begin(), cl_parameter_proxies_.end(),
									  findCLProxyByName(optionName))) == cl_parameter_proxies_.end()) {
