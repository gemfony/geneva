...
public:
	...
	GRandomFactory(void) throw() :
		g01_(DEFAULTFACTORYBUFFERSIZE), seed_(GSeed()), n01Threads_(
				DEFAULT01PRODUCERTHREADS), maxNThreads_(DEFAULTMAXNTHREADS) {
	...
	GRandomFactory(boost::uint8_t n01Threads) throw() :
		g01_(DEFAULTFACTORYBUFFERSIZE), seed_(GSeed()), n01Threads_(
				n01Threads ? n01Threads : 1), maxNThreads_(DEFAULTMAXNTHREADS) {
		startProducerThreads();
	}
	...
	void setNProducerThreads(boost::uint8_t n01Threads) {
		...
		if (localN01Threads > maxNThreads_)
			localN01Threads = maxNThreads_;
	 ...
	 */
	boost::shared_ptr<GRandomNumberContainer_dbl> new01Container(void) {
...
private:
	...
	void startProducerThreads(void) throw() {
		for (boost::uint8_t i = 0; i < n01Threads_; i++) {
			...
			producer_threads_01_.create_thread(boost::bind(
					&GRandomFactory::producer01, this, seed_ + boost::uint32_t(
							i)));
		}
	}
	...
	void producer01(const boost::uint32_t& seed) throw() {
		try {
			...
			while (true) {
				...
				if (g01_.remainingSpace()) { // any space left ?
	...
	Gem::Util::GBoundedBuffer<boost::shared_ptr<GRandomNumberContainer_dbl> >
			g01_;
...
public:
	 ...
	 */
	double evenRandom(double min, double max) {
#ifdef DEBUG
		// Check that min and max have appropriate values
		assert(min<=max);
#endif
	...
	double gaussRandom(double mean, double sigma) {
		return sigma * sqrt(fabs(-2. * log(1. - evenRandom()))) * sin(2. * M_PI
				* evenRandom()) + mean;
	}
	 ...
	 */
	double doubleGaussRandom(double mean, double sigma, double distance) {
	 ...
	 */
	boost::uint16_t discreteRandom(boost::uint16_t max) {
		boost::uint16_t result =
				static_cast<boost::uint16_t> (GRandom::evenRandom(
	 ...
	 */
	boost::int16_t discreteRandom(boost::int16_t min, boost::int16_t max) {
#ifdef DEBUG
		assert(min < max);
#endif
		boost::int16_t result = discreteRandom(static_cast<boost::int16_t> (max
				- min)) + min;
	 ...
	 */
	GenEvA::bit bitRandom(double probability) {
#ifdef DEBUG
		assert(probability>=0 && probability<=1);
#endif
		return (GRandom::evenRandom() < probability ? GenEvA::TRUE
				: GenEvA::FALSE);
	}
	 ...
	 */
	char charRandom(bool printable = true) {
		if (!printable) {
