...
/**
 * Class to be thrown as a message in the case of a time-out
 */
class time_out: public boost::exception {
};

typedef boost::error_info<struct tag_string,string> string_info_; ///< String container for error messages
 ...
 */
template<class T>
class GBoundedBuffer: private boost::noncopyable {
public:
...
typedef	typename container_type::size_type size_type;
	...
	~GBoundedBuffer() throw()
	{
		...
		// This is a standard error raised by the lock/mutex
			...
			GLogStreamer gls;
			gls << "Caught thread_resource_error in GBoundedBuffer::~GBoundedBuffer(). Terminating ..." << endl
			<< logLevel(CRITICAL);
		...
		// We do not know whether any of the destructors of the items in the buffer throw anything
			...
			GLogStreamer gls;
			gls << "Caught unknown exception in GBoundedBuffer::~GBoundedBuffer(). Terminating ..." << endl
			<< logLevel(CRITICAL);
	...
	virtual void push_front(const value_type& item, uint16_t sec, uint16_t msec)
	{
		...
		if (container_.size() >= capacity_) {
			...
			// Note that the additional check for the wake-up condition is
			// due to the possibility of spurious wake-ups inherent to the
			// pthread-library (which on some platforms is the base of Boost.threads).
			// See the pthread description of this issue for further information.
			if(!not_full_.timed_wait(lock,timeout) || container_.size() >= capacity_)
			throw time_out(); // << string_info_("GBoundedBuffer::push_front() time out");
		}
