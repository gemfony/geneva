...
public:
    ...
    public:
        ...
        param_type(const param_type &params)
          ...
          , m_distance(params.m_distance) { /* nothing */
        }
        ...
        bool operator==(const param_type &p) const {
            if(m_mean != p.m_mean)
                return false;
            if(m_sigma1 != p.sigma1())
                return false;
            if(m_sigma2 != p.sigma2())
                return false;
    ...
    bi_normal_distribution(fp_type mean, fp_type sigma1, fp_type sigma2, fp_type distance)
      ...
      , m_params_store(m_params) { /* nothing */
    }
    ...
    bi_normal_distribution(bi_normal_distribution<fp_type> const &cp)
      ...
      , m_params_store(cp.m_params_store) { /* nothing */
    }
...
private:
    std::normal_distribution<fp_type>
        m_normal_distribution{}; ///< Needed to form each gaussian "hill" of the distribution
    std::bernoulli_distribution
        m_uniform_bool{}; ///< Needed to decide whether a gaussian is created for the left or right peak
    ...
    const param_type m_params_store{
        m_params
