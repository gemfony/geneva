...
public:
    ...
    /**
	  ...
	  */
    GWebsocketClientT( std::string                    address,
                       unsigned short                 port,
        ...
        f_when_control_frame_arrived = [this]( frame_type frame_t, string_view s ) {
            ...
            if ( this->m_verbose_control_frames ) {
                if ( boost::beast::websocket::frame_type::close == frame_t ) {
                    glogger << "GWebsocketClientT<> client has received a close frame" << std::endl << GLOGGING;
                }
                else if ( boost::beast::websocket::frame_type::ping == frame_t ) {
                    glogger << "GWebsocketClientT<> client has received a ping frame" << std::endl << GLOGGING;
                }
                else if ( boost::beast::websocket::frame_type::pong == frame_t ) {
                    glogger << "GWebsocketClientT<> client has received a pong frame" << std::endl << GLOGGING;
                }
            }
        };
    ...
    {
        glogger << std::endl
                << "GWebsocketClientT<> is shutting down. Processed " << this->getNProcessed() << " items in total"
                << std::endl
...
private:
    ...
    run_() override
    {
        ...
        glogger << "GWebsocketClientT<processable_type>::run_(): Client session has terminated" << std::endl
                << GLOGGING;
    }
    ...
    async_start_write( const std::string & message )
    {
        ...
        // Send the message
        auto self = this->shared_from_this();
        m_ws.async_write( boost::asio::buffer( m_outgoing_message ),
    ...
    async_start_read()
    {
        ...
        auto self = this->shared_from_this();
        m_ws.async_read( m_incoming_buffer, [self]( boost::system::error_code ec, std::size_t nBytesTransferred ) {
    ...
    when_connected( boost::system::error_code ec )
    {
        ...
        auto self = this->shared_from_this();
        m_ws.async_handshake( m_address, "/", [self]( boost::system::error_code ec ) {
    ...
    void
    when_handshake_complete( boost::system::error_code ec )
    {
        if ( ec ) {
            glogger << "In GWebsocketClientT<processable_type>::when_handshake_complete():" << std::endl
        ...
        async_start_write( Gem::Courtier::container_to_string(
    ...
    )
    {
        ...
        try {
            ...
            auto self = this->shared_from_this();
            m_gtp.async_schedule( [self]() { self->process_request(); } );
    ...
    process_request()
    {
        ...
        Gem::Courtier::container_from_string( message, m_command_container, m_serialization_mode );  // may throw
    ...
    boost::beast::websocket::stream<socket> m_ws {
        m_io_context };  ///< All messages are sent and received through this socket
    ...
    Gem::Common::GThreadPool m_gtp {
        1 };  ///< Holds workers doing the processing and serialization of incoming workloads

    std::atomic<bool> m_processing_is_active {
        false };  ///< A safeguard against accidental processing of two work items
...
public:
    ...
    /**
	  ...
	  */
    GWebsocketConsumerSessionT( boost::asio::io_context &                                io_context,
        ...
        , m_verbose_control_frames( verbose_control_frames )
    {
        ...
        f_when_control_frame_arrived = [this]( frame_type frame_t, string_view s ) {
            ...
            if ( this->m_verbose_control_frames ) {
                if ( boost::beast::websocket::frame_type::close == frame_t ) {
                    glogger << "GWebsocketConsumerSessionT<> session has received a close frame" << std::endl
                            << GLOGGING;
                }
                else if ( boost::beast::websocket::frame_type::ping == frame_t ) {
                    glogger << "GWebsocketConsumerSessionT<> session has received a ping frame" << std::endl
                            << GLOGGING;
                }
                else if ( boost::beast::websocket::frame_type::pong == frame_t ) {
                    glogger << "GWebsocketConsumerSessionT<> session has received a pong frame" << std::endl
                            << GLOGGING;
                }
            }
        };
    ...
    GWebsocketConsumerSessionT<processable_type> & operator=( const GWebsocketConsumerSessionT<processable_type> & )
        = delete;
    GWebsocketConsumerSessionT<processable_type> & operator=( GWebsocketConsumerSessionT<processable_type> && )
        = delete;
...
private:
    ...
    async_start_ping()
    {
        ...
        auto self = this->shared_from_this();
        m_timer.async_wait( boost::asio::bind_executor( m_strand, [self]( boost::system::error_code ec ) {
        ...
        m_ws.async_ping( m_ping_data, boost::asio::bind_executor( m_strand, [self]( boost::system::error_code ec ) {
    ...
    async_start_accept()
    {
        ...
        auto self = this->shared_from_this();
        m_ws.async_accept( boost::asio::bind_executor( m_strand, [self]( boost::system::error_code ec ) {
    ...
    /**
	  ...
	  */
    void
    when_ping_sent( boost::system::error_code ec )
    {
        if ( ec ) {
            if ( ec != boost::asio::error::operation_aborted ) {
                glogger << "GWebsocketConsumerSessionT<processable_type>::when_ping_sent(): " << ec.message()
                        << std::endl
                        << GLOGGING;
            }
    ...
    /**
	  ...
	  */
    void
    when_timer_fired( boost::system::error_code ec )
    {
        if ( ec ) {
            if ( ec != boost::asio::error::operation_aborted ) {
                glogger << "GWebsocketConsumerSessionT<processable_type>::when_timer_fired(): " << ec.message()
                        << std::endl
                        << GLOGGING;
            }
        ...
        if ( m_ping_state == beast_ping_state::CONNECTION_IS_ALIVE ) {
            ...
            if ( not this->m_check_server_stopped() ) {
                ...
                glogger << "GWebsocketConsumerSessionT<processable_type>::when_timer_fired():" << std::endl
    ...
    /**
	  ...
	  */
    void
    when_connection_accepted( boost::system::error_code ec )
    {
        if ( ec ) {
            glogger << "GWebsocketConsumerSessionT<processable_type>::when_connection_accepted(): " << ec.message()
                    << std::endl
                    << GLOGGING;
    ...
    )
    {
        if ( ec ) {
            if ( ec != boost::beast::websocket::error::closed ) {
                glogger << "GWebsocketConsumerSessionT<processable_type>::when_read(): " << ec.message() << std::endl
                        << GLOGGING;
            }
    ...
    )
    {
        if ( ec ) {
            if ( ec != boost::beast::websocket::error::closed ) {
                glogger << "GWebsocketConsumerSessionT<processable_type>::when_written(): " << ec.message() << std::endl
                        << GLOGGING;
            }
    ...
    /**
	  ...
	  */
    void
    do_close( boost::beast::websocket::close_code cc )
    {
    ...
    process_request()
    {
        try {
            ...
            switch ( inboundCommand ) {
                ...
                case networked_consumer_payload_command::RESULT: {
                    ...
                    if ( payload_ptr ) {
                        ...
                        glogger << "GWebsocketConsumerSessionT<processable_type>::process_request():" << std::endl
            ...
            glogger << "GWebsocketConsumerSessionT<processable_type>::process_request(): Caught exception" << std::endl
                    << GLOGGING;
...
private:
                   ...
                   boost::program_options::options_description & hidden ) override
    {
        ...
        hidden.add_options()
            ...
            ("beast_nListenerThreads", po::value<std::size_t>( &m_n_threads )->default_value( m_n_threads ),
              "\t[beast] The number of threads used to listen for incoming connections" )
            ("beast_use_pinning", po::value<bool>( &m_use_pinning )->default_value(false), "\t[asio] Whether to pin each thread to a given core")
            ("beast_use_multiple_io_contexts", po::value<bool>( &m_use_multiple_io_contexts )->default_value(false), "\t[asio] Whether to use one io_context-object for each run()-call")
            ("beast_set_no_delay", po::value<bool>( &m_use_no_delay_option )->default_value(false), "\t[asio] Whether to set the no_delay option on sockets")
    ...
    async_startProcessing_() override
    {
        ...
        if ( ec || not m_acceptor.is_open() ) {
            if ( ec ) {
                throw gemfony_exception(
                    ...
                    << "GWebsocketConsumerT<>::async_startProcessing_() / m_acceptor.open: Got error message \""
                    << ec.message() << "\"" << std::endl
        ...
        if ( ec ) {
            throw gemfony_exception(
                g_error_streamer( DO_LOG, time_and_place )
                << "GWebsocketConsumerT<>::async_startProcessing_() / m_acceptor.bind: Got error message \""
                << ec.message() << "\"" << std::endl
                << "No connections will be accepted. The server is not running" << std::endl );
        }
        ...
        if ( ec ) {
            throw gemfony_exception(
                g_error_streamer( DO_LOG, time_and_place )
                << "GWebsocketConsumerT<>::async_startProcessing_() / m_acceptor.listen: Got error message \""
                << ec.message() << "\"" << std::endl
                << "No connections will be accepted. The server is not running" << std::endl );
        }
    ...
    {
        if ( ec ) {
            glogger << "In GWebsocketConsumerT<>::when_accepted(): Got error code \"" << ec.message() << "\""
                    << std::endl
    ...
    /**
	  ...
	  */
    std::shared_ptr<GBaseClientT<processable_type>>
    getClient_() const override
    {
        return std::shared_ptr<GBaseClientT<processable_type>>(
            new GWebsocketClientT<processable_type>( m_server,
                                                     m_port,
                                                     m_serializationMode,
                                                     m_verbose_control_frames ) );
    }
    ...
    boost::asio::ip::tcp::endpoint m_endpoint { boost::asio::ip::tcp::v4(),
                                                m_port };  ///< The endpoint is reset in async_startProcessing
