...
// Short alias for the consumer namespace, visible library-wide (this header
// is included transitively by every consumer and by the broker). Forward-
// declared so the alias resolves before the real definitions below.
...
public:
    ...
    GBaseConsumerT(const GBaseConsumerT<processable_type> &) =
    ...
    GBaseConsumerT<processable_type> &
    operator=(const GBaseConsumerT<processable_type> &) = delete; ///< Intentionally left undefined
    GBaseConsumerT<processable_type> &
    operator=(GBaseConsumerT<processable_type> &&) = delete; ///< Intentionally left undefined
...
protected:
    ...
    /**
	  ...
	  */
    virtual void
    addConfigurationOptions(Gem::Common::GParserBuilder &gpb) { /* nothing -- no local data */
    }
