...
public:
   ...
   G_API GBaseClientT()
	 ...
	 */
   G_API GBaseClientT(boost::shared_ptr<processable_type> additionalDataTemplate)
		: startTime_(boost::posix_time::microsec_clock::local_time())
		, maxDuration_(boost::posix_time::microsec(0))
		, processed_(0)
		, processMax_(0)
	 ...
	 */
	G_API void setProcessMax(const boost::uint32_t& processMax){
	 ...
	 */
	G_API boost::uint32_t getProcessMax() const{
		return processMax_;
	}
	 ...
	 */
	G_API void setMaxTime(const boost::posix_time::time_duration& maxDuration) {
	 ...
	 */
	G_API boost::posix_time::time_duration getMaxTime() {
		return maxDuration_;
	}
...
protected:
	...
	virtual G_API bool init() { return true; }

	/***************************************************************************/

	/** @brief Perform necessary finalization activities */
	virtual G_API bool finally() { return true; }

	/***************************************************************************/

	/** @brief Retrieve work items from the server. To be defined by derived classes. */
	virtual G_API bool retrieve(std::string&, std::string&, std::string&) = 0;

	/***************************************************************************/

	/** @brief Submit processed items to the server. To be defined by derived classes. */
	virtual G_API bool submit(const std::string&, const std::string&) = 0;

	/***************************************************************************/

	/** @brief Custom halt condition for processing */
	virtual G_API bool customHalt(){ return false; }
