...
public:
	...
	virtual void load(const GObject* cp){
		...
		const GParameterCollectionT<T> *gpct	= dynamic_cast<const GParameterCollectionT<T> *> (cp);

		// dynamic_cast will emit a NULL pointer, if the conversion failed
		if (!gpct) {
			std::ostringstream error;
			error << "In GParameterCollectionT<T>::load(): Conversion error!" << std::endl;
		...
		if (gpct == this) {
			std::ostringstream error;
			error << "In GParameterCollectionT<T>::load(): Error!" << std::endl
