...
CONFIG_DIRS_EX=`find $GENEVA_LIBS -type d -path '*/examples/*' -name config 2>/dev/null | sort`
CONFIG_DIRS_TST=`find $GENEVA_LIBS -type d \( -path '*/tests/*' -o -path '*/benchmarks/*' \) -name config 2>/dev/null | sort`
