...
int main()
{
    ...
    std::cout << std::left
              << std::setw(22) << "Function"
              << std::right
              << std::setw(14) << "MaxRelErr"
              << std::setw(14) << "MeanRelErr"
              << "  Status\n"
              << std::string(56, '-') << "\n";
    ...
    for (int fi = 0; fi < N_FUNCTIONS; ++fi) {
        ...
        for (int i = 0; i < N_INDIVIDUALS; ++i) {
            cpu_results[static_cast<std::size_t>(i)] =
                BM::eval(info.id,
                         h_params.data() + static_cast<std::ptrdiff_t>(i * DIM),
                         DIM);
        }
        ...
        try {
            Gem::Geneva::batchEvalBenchmarkGPU(
                h_params.data(), gpu_results.data(), N_INDIVIDUALS, DIM, info.id);
        ...
        std::cout << std::left  << std::setw(22) << info.name
                  << std::right << std::setw(14) << std::scientific
                               << std::setprecision(3) << max_rel
