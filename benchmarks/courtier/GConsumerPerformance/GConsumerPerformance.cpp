...
#ifdef DEBUG
    gbscmode = Gem::Common::narrow<GCPModes>(tmp);
#else
...
bool parseCommandLine(
    ...
    gpb.registerCLParameter<GCPModes>(
        ...
        "Whether to run this program with a serial consumer (0), multi-threaded (1), internal "
        "serial networking (2), serial networking (3), "
        "multithreaded and internal serial networking (4), multithreaded and serial networked mode "
        "(5),"
        "internal async networking (6), async networking (7), multithreaded and internal async "
        "networking (8) or multithreaded and async networking (9)"
    );
    ...
    gpb.registerCLParameter<bool>(
        ...
        "effect for modes requiring a server. You can either say \"--serverMode=true\", just "
        "\"--serverMode\" or simple \"-s\".",
    ...
    gpb.registerCLParameter<unsigned short>("port", port, DEFAULTPORTAP, "The port on the server");
    ...
    gpb.registerCLParameter<bool>(
        ...
        ,
        "Indicates whether producers should connect directly to the broker or through the broker "
        "connector object",
...
void connectorProducer(
    ...
    while(cycleCounter++ < nProductionCycles) {
        ...
        nSentItems += Gem::Common::narrow<std::uint32_t>(data.size());
        ...
        if(!status.is_complete) {
            std::size_t n_erased = std::erase_if(data, [](std::shared_ptr<WORKLOAD> p) -> bool {
                return (p->getProcessingStatus() == Gem::Courtier::processingStatus::DO_PROCESS);
            });

#ifdef DEBUG
            glogger << "In connectorProducer(): " << std::endl
        ...
        nReceivedItemsNew += Gem::Common::narrow<std::uint32_t>(data.size());
        nReceivedItemsOld += Gem::Common::narrow<std::uint32_t>(oldWorkItems.size());
    ...
    std::cout << "connectorProducer " << id << " has finished." << std::endl
              ...
              << "Missing = " << nSentItems - (nReceivedItemsNew + nReceivedItemsOld) << std::endl
              << std::flush;
}
...
void brokerProducer(
    ...
    broker<WORKLOAD>()->enrol(CurrentBufferPort);
...
int main(int argc, char **argv) {
    ...
    if(useDirectBrokerConnection) {
        producer_gtg.create_threads(
            std::bind(brokerProducer, nProductionCycles, nContainerObjects, nContainerEntries),
            nProducers
        );
    }
    else {
    ...
    case GCPModes::INTERNALSERIALNETWORKING: {
        ...
        for(std::size_t worker = 0; worker < nWorkers; worker++) {
            std::shared_ptr<GAsioSerialTCPClientT<WORKLOAD>> p(
                new GAsioSerialTCPClientT<WORKLOAD>("localhost", Gem::Common::to_string(port))
            );
            clients.push_back(p);
    ...
    case GCPModes::THREADANDINTERNALSERIALNETWORKING: {
        ...
        broker<WORKLOAD>()->enrol(consumers);
    ...
    case GCPModes::INTERNALASYNCNETWORKING: {
        ...
        clients.clear();
        for(std::size_t worker = 0; worker < nWorkers; worker++) {
            std::shared_ptr<GAsioAsyncTCPClientT<WORKLOAD>> p(
                new GAsioAsyncTCPClientT<WORKLOAD>("localhost", Gem::Common::to_string(port))
            );
            clients.push_back(p);
    ...
    if(executionMode == GCPModes::INTERNALSERIALNETWORKING ||
