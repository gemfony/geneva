...
public:
	 ...
	 decltype(boost::this_thread::get_id()) getId() const { return boost::this_thread::get_id(); }
};
...
void runner() {
	...
	if(g_tlsTest.getId() != boost::this_thread::get_id()) {
		std::cout << "Got invalid thread id from tls: " << g_tlsTest.getId() << " (expected " << boost::this_thread::get_id() << ")" << std::endl;
	}
}

int main(int argc, char **argv) {
	std::vector<boost::thread> v_thread;
	for(std::size_t t=0; t<NTHREADS; t++) {
		v_thread.emplace_back(boost::thread(runner));
	}
