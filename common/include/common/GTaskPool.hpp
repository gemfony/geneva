 ...
 * multi-consumer task queue (GBlockingMPMCQueueT). It is a modernised, dependency-
 * lean alternative to GThreadPool: it carries no Boost.ASIO dependency at all (no
 * io_context, no work guard), and uses the queue's close() for clean, deadlock-free
 * shutdown -- workers block at ~0 CPU on an empty queue and wake the instant work
 * is submitted or the pool is shut down.
 *
 * The public interface mirrors GThreadPool so it can act as a drop-in replacement:
 * construct with a thread count, submit work with async_schedule() (returning a
 * std::future), and wait() for the queue to run empty. Workers are started eagerly
 * in the constructor, so getNThreads() always reports the live count.
 *
 * Note (vs. GThreadPool): the submitted callable plus its bound arguments must be
 * copyable, because tasks are type-erased through std::function. Geneva's usage
 * (closures capturing std::shared_ptr) satisfies this. If move-only tasks are ever
 * needed, the std::function task type can be swapped for a move-only type-erased
 * wrapper without touching the public API.
 *
 * Like GThreadPool, this class is neither copyable nor movable, and wait() /
 * setNThreads() must NOT be called from inside a task running in the pool (and a
 * task must not submit further work while another thread is inside wait()): both
 * would deadlock, exactly as for GThreadPool.
 */
class GTaskPool final {
public:
    /***************************************************************************/
    ...
    explicit GTaskPool(unsigned int n_threads);
    ...
    GTaskPool(GTaskPool const &) = delete;
    GTaskPool &operator=(GTaskPool const &) = delete;
    GTaskPool(GTaskPool &&) = delete;
    GTaskPool &operator=(GTaskPool &&) = delete;
    ...
    auto async_schedule(F &&f, Args &&...args)
        ...
        auto task = [promise_ptr,
            ...
            try {
                ...
                try {
                    ...
                    glogger << "In GTaskPool::async_schedule(): promise.set_exception() threw."
                            << '\n'
                            << "We cannot continue" << '\n'
                            << GTERMINATION;
                }
        ...
        if(not enqueue(std::function<void()>(std::move(task)))) {
            ...
            promise_ptr->set_exception(std::make_exception_ptr(geneva_exception(
                ...
                << "In GTaskPool::async_schedule(): submission after the pool was closed" << '\n'
            )));
        }
        return result;
    }
    ...
    void post(F &&f, Args &&...args) {
        auto task = [f = std::forward<F>(f),
            ...
            try {
                ...
                glogger << "In GTaskPool::post(): a fire-and-forget task threw:" << '\n'
                        << e.what() << '\n'
                        << "No future to carry it -- logged and ignored." << '\n'
                        << GWARNING;
            }
            catch(...) {
                glogger << "In GTaskPool::post(): a fire-and-forget task threw a"
                        << " non-std exception." << '\n'
                        << "No future to carry it -- logged and ignored." << '\n'
                        << GWARNING;
            }
        ...
        if(not enqueue(std::function<void()>(std::move(task)))) {
            glogger << "In GTaskPool::post(): submission after the pool was closed;"
                    << " task dropped." << '\n'
                    << GWARNING;
        }
    }
