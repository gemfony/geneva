...
int Go2::clientRun_() {
    ...
    std::shared_ptr<Gem::Courtier::GBaseClientT<gpar::GTreeGenome>> p =
 ...
 */
void Go2::registerContentCreator(const std::shared_ptr<Gem::Common::GFactoryT<gpar::GTreeGenome>> &cc_ptr) {
    if(not cc_ptr) {
        throw geneva_exception(
            g_error_streamer(DO_LOG, Gem::Common::timeAndPlace())
        ...
        if(this->empty()) {
            if(content_creator_ptr_) {
                for(std::size_t ind = 0; ind < algorithms_cnt_.at(0)->getDefaultPopulationSize();
                    ind++) {
                    std::shared_ptr<gpar::GTreeGenome> p_ind = (*content_creator_ptr_)();
                    if(p_ind) {
...
void Go2::runAlgorithmChain(std::uint32_t first_algorithm_offset) {
    ...
    for(const auto &alg_ptr : algorithms_cnt_) {
        ...
        if(copy_best_individuals_only_) {
            for(const auto &best_ind_ptr : alg_ptr->getBestGlobalIndividuals<gpar::GTreeGenome>()) {
 ...
 */
std::shared_ptr<gpar::GTreeGenome> Go2::getBestGlobalIndividual_() const {
    ...
    return this->front()->clone<gpar::GTreeGenome>();
}
 ...
 * @return The best individual found
 */
std::vector<std::shared_ptr<gpar::GTreeGenome>> Go2::getBestGlobalIndividuals_() const {
    // Do some error checking
    if(this->empty()) {
        throw geneva_exception(
            g_error_streamer(DO_LOG, Gem::Common::timeAndPlace())
    ...
    std::size_t pos = 0;
    std::vector<std::shared_ptr<gpar::GTreeGenome>> best_individuals;
    for(const auto &ind_ptr : *this) {
        ...
        best_individuals.push_back(ind_ptr->clone<gpar::GTreeGenome>());
 ...
 * @return The best individual found
 */
std::shared_ptr<gpar::GTreeGenome> Go2::getBestIterationIndividual_() const {
    throw geneva_exception(
        g_error_streamer(DO_LOG, Gem::Common::timeAndPlace())
 ...
 * @return The best individual found
 */
std::vector<std::shared_ptr<gpar::GTreeGenome>> Go2::getBestIterationIndividuals_() const {
    throw geneva_exception(
        g_error_streamer(DO_LOG, Gem::Common::timeAndPlace())
