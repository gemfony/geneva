 ...
 */
GCandidateSolution::GCandidateSolution() = default;
 ...
 */
GCandidateSolution::GCandidateSolution(const std::size_t n_fitness_criteria)
 ...
 */
GCandidateSolution::GCandidateSolution(GCandidateSolution const &cp)
  : Gem::Courtier::GProcessable(cp)
  , Gem::Common::GCommonInterfaceT<GCandidateSolution>(cp)
 ...
 */
void GCandidateSolution::setPolicy(std::shared_ptr<GProblemPolicy> policy) {
    if(not policy) {
        throw geneva_exception(
            g_error_streamer(DO_LOG, Gem::Common::timeAndPlace())
            << "In GCandidateSolution::setPolicy(): Error!" << '\n'
 ...
 */
void GCandidateSolution::clear_stored_results_vec() {
...
GCandidateSolution::markAsProcessedWith(std::vector<individual_processing_result> const &result_cnt) {
#ifdef DEBUG
    if(result_cnt.size() != stored_results_cnt_.size()) {
        throw geneva_exception(
            g_error_streamer(DO_LOG, Gem::Common::timeAndPlace())
            << "In GCandidateSolution::markAsProcessedWith(): Vector dimensions" << '\n'
 ...
 */
individual_processing_result GCandidateSolution::getStoredResult(const std::size_t id) const {
    if(not this->is_processed()) {
        throw geneva_exception(
            g_error_streamer(DO_LOG, Gem::Common::timeAndPlace())
            << "In GCandidateSolution::getStoredResult(): Tried to" << '\n'
 ...
 */
individual_processing_result
GCandidateSolution::process(const std::vector<individual_processing_result> &res_vec) {
    using Gem::Courtier::processingStatus;

    if(processingStatus::DO_PROCESS != processing_status_) {
        throw geneva_exception(
            g_error_streamer(DO_LOG, Gem::Common::timeAndPlace())
            << "In GCandidateSolution::process(): Function called while processing_status_ was set to "
            << processing_status_ << '\n'
    ...
    try {
        ...
        error_description_stream << "In GCandidateSolution::process():" << '\n'
    ...
    if(this->has_errors()) { // Either an exception was caught or the user flagged an error
        ...
        throw g_candidate_processing_exception(
 ...
 */
void GCandidateSolution::runEvaluation_(const std::vector<individual_processing_result> &res_vec) {
    // Find out whether this is a valid solution (must be called first, to fill validity_level_).
    if(this->checkFeasibility_(validity_level_) ||
        ...
        try {
            if(not res_vec.empty()) {
                if(res_vec.size() != this->getNStoredResults()) {
                    throw geneva_exception(
                        g_error_streamer(DO_LOG, Gem::Common::timeAndPlace())
                        << "In GCandidateSolution::runEvaluation_(): Error!" << '\n'
 ...
 */
void GCandidateSolution::setFitness_(std::vector<double> const &f_cnt) {
#ifdef DEBUG
    if(f_cnt.size() != this->getNStoredResults()) {
        throw geneva_exception(
            g_error_streamer(DO_LOG, Gem::Common::timeAndPlace())
            << "In GCandidateSolution::setFitness_(): Error!" << '\n'
 ...
 */
void GCandidateSolution::setResult(const std::size_t id, const double value) {
#ifdef DEBUG
    if(id >= this->getNStoredResults()) {
        throw geneva_exception(
            g_error_streamer(DO_LOG, Gem::Common::timeAndPlace())
            << "In GCandidateSolution::setResult(): Error!" << '\n'
 ...
 */
std::tuple<double, double> GCandidateSolution::getFitnessTuple(const std::uint32_t id) const {
 ...
 */
bool GCandidateSolution::isGoodEnough(std::vector<double> const &boundaries) {
#ifdef DEBUG
    if(boundaries.size() != this->getNStoredResults()) {
        throw geneva_exception(
            g_error_streamer(DO_LOG, Gem::Common::timeAndPlace())
            << "In GCandidateSolution::isGoodEnough(): Error!" << '\n'
 ...
 */
bool GCandidateSolution::isValid() const {
#ifdef DEBUG
    if(this->is_due_for_processing() || this->has_errors()) {
        throw geneva_exception(
            g_error_streamer(DO_LOG, Gem::Common::timeAndPlace())
            << "In GCandidateSolution::isValid():" << '\n'
 ...
 */
bool GCandidateSolution::randomInit(activityMode const &am) {
 ...
 */
std::vector<double> GCandidateSolution::raw_fitness_vec_() const {
 ...
 */
std::vector<double> GCandidateSolution::transformed_fitness_vec_() const {
    std::vector<double> result_vec;
    for(std::size_t i = 0; i < this->getNStoredResults(); i++) {
 ...
 */
void GCandidateSolution::preProcess_() {
 ...
 */
void GCandidateSolution::postProcess_() {
 ...
 */
void GCandidateSolution::setAllFitnessTo(const double raw_value, const double transformed_value) {
    for(std::size_t i = 0; i < this->getNStoredResults(); i++) {
 ...
 */
double GCandidateSolution::sumCombiner() const {
    double result = 0.;
 ...
 */
double GCandidateSolution::fabsSumCombiner() const {
    double result = 0.;
    for(std::size_t id = 0; id < this->getNStoredResults(); id++) {
 ...
 */
double GCandidateSolution::squaredSumCombiner() const {
    double result = 0.;
    for(std::size_t id = 0; id < this->getNStoredResults(); id++) {
 ...
 */
double GCandidateSolution::weighedSquaredSumCombiner(std::vector<double> const &weights) const {
    if(this->getNStoredResults() != weights.size()) {
        throw geneva_exception(
            g_error_streamer(DO_LOG, Gem::Common::timeAndPlace())
            << "In GCandidateSolution::weighedSquaredSumCombiner(): Error!" << '\n'
 ...
 */
void GCandidateSolution::addConfigurationOptions_(Gem::Common::GParserBuilder &gpb) {
    // Call our CRTP base class's function (the category root has no GObject parent).
    Gem::Common::GCommonInterfaceT<GCandidateSolution>::addConfigurationOptions_(gpb);
...
/******************************************************************************/
/**
 * @brief Loads the data of another GCandidateSolution.
 * @param cp The source candidate whose data is copied into this one
 */
void GCandidateSolution::load_(const GCandidateSolution *cp) {
    const auto *p_load =
        Gem::Common::g_convert_and_compare<GCandidateSolution, GCandidateSolution>(cp, this);
 ...
 */
void GCandidateSolution::compare_(
    ...
    GToken token("GCandidateSolution", e);

    // Compare our CRTP base data (the category root has no GObject parent) ...
    Gem::Common::compare_base_t<Gem::Common::GCommonInterfaceT<GCandidateSolution>>(*this, *p_load, token);
