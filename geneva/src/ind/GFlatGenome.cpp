...
/******************************************************************************/
/**
 * Installs the value arrays + shared layout produced by a GGenomeBuilder and seeds the per-group
 * adaption state in the auxiliary store from the layout configuration.
 */
...
template <typename T>
std::size_t GFlatGenome::adaptFPChannel(std::vector<T> &store, ChannelLayout<T> const &ch, AuxKey key) {
    ...
    for(std::size_t gi = 0; gi < ch.groups.size(); ++gi) {
        ...
        n += adaptGaussGroup<T>(g.gauss, states[gi], vals, g.range, gr_);
    }
    return n;
}
...
template <typename T>
void GFlatGenome::resetFPChannel(ChannelLayout<T> const &ch, AuxKey key) {
    if(not this->hasAux(key)) {
        return;
    }
    std::span<GaussState<T>> states = this->metaRecords<GaussState<T>>(key);
...
/******************************************************************************/
/**
 * Retrieves information from the adaption state with a given property. Currently the "sigma" property
 * of the FP Gauss adaptors is exposed (one entry per Gauss group), which is what the pluggable
 * monitors read; other properties are silently ignored.
 */
void GFlatGenome::queryAdaptor(
    ...
    auto collect =
        [&]<typename T>(ChannelLayout<T> const &ch, AuxKey key, const char *name) {
            if(adaptor_name != name || not this->hasAux(key)) {
                return;
            }
            std::span<const GaussState<T>> states = this->metaRecords<GaussState<T>>(key);
            for(std::size_t gi = 0; gi < ch.groups.size(); ++gi) {
                if(ch.groups[gi].has_gauss) {
