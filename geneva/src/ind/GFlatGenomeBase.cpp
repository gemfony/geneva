 ...
 */
GFlatGenomeBase::GFlatGenomeBase() = default;
 ...
 */
GFlatGenomeBase::GFlatGenomeBase(const std::size_t n_fitness_criteria)
  : GCandidateSolution(n_fitness_criteria) {
    /* nothing */
}
 ...
 * @param cp A constant reference to another GFlatGenomeBase object to be copied
 */
GFlatGenomeBase::GFlatGenomeBase(GFlatGenomeBase const &cp)
  : GCandidateSolution(cp)
  , dv_(cp.dv_)
  , fv_(cp.fv_)
  , iv_(cp.iv_)
  , bv_(cp.bv_)
 ...
 */
void GFlatGenomeBase::setGenome(GenomeData const &g) {
        ...
        for(std::size_t k = 0; k < ch.size(); ++k) {
            if(ch.fold[k] && ch.lower[k] > ch.upper[k]) {
                throw geneva_exception(
                    ...
                    << "In GFlatGenomeBase::setGenome(): Error!" << '\n'
...
/******************************************************************************/
/**
 * @brief Feasibility check via the shared policy (the constraint reads this genome's parameter values).
 ...
 */
bool GFlatGenomeBase::checkFeasibility_(double &validity_level) const {
    return this->policy().fulfillsConstraints(*this, validity_level);
}

/******************************************************************************/
/**
 * @brief Searches for compliance with expectations with respect to another object of the same type.
 * @param cp A constant reference to another GFlatGenomeBase, camouflaged as a GCandidateSolution
 * @param e The expected outcome of the comparison
 * @param limit The maximum deviation tolerated for floating point comparisons (unused here)
 */
void GFlatGenomeBase::compare_(
    ...
    const GFlatGenomeBase *p_load =
        Gem::Common::g_convert_and_compare<GCandidateSolution, GFlatGenomeBase>(cp, this);

    GToken token("GFlatGenomeBase", e);

    Gem::Common::compare_base_t<GCandidateSolution>(*this, *p_load, token);
...
/******************************************************************************/
/**
 * @brief Loads the data of another GFlatGenomeBase object, camouflaged as a GCandidateSolution.
 * @param cp A pointer to another GFlatGenomeBase object, camouflaged as a GCandidateSolution
 */
void GFlatGenomeBase::load_(const GCandidateSolution *cp) {
    const GFlatGenomeBase *p_load =
        Gem::Common::g_convert_and_compare<GCandidateSolution, GFlatGenomeBase>(cp, this);

    GCandidateSolution::load_(cp);
 ...
 */
bool GFlatGenomeBase::randomInit_(activityMode const &am) {
    bool modified = false;
 ...
 */
template <typename T>
bool GFlatGenomeBase::randomInitFP(std::vector<T> &store, ChannelLayout<T> const &ch, activityMode const &am) {
    bool modified = false;
 ...
 * @return true if at least one entry was modified, false otherwise
 */
bool GFlatGenomeBase::randomInitInt(activityMode const &am) {
    bool modified = false;
 ...
 * @return true if at least one entry was modified, false otherwise
 */
bool GFlatGenomeBase::randomInitBool(activityMode const &am) {
    bool modified = false;
 ...
 */
std::size_t GFlatGenomeBase::getCrossOverPos(const std::size_t lower, const std::size_t upper) {
    if(lower == 0) {
        throw geneva_exception(
            g_error_streamer(DO_LOG, Gem::Common::timeAndPlace())
            << "In GFlatGenomeBase::getCrossOverPos(): Error!" << '\n'
 ...
 * @param cp_base A constant reference to the other parent (a GFlatGenomeBase)
 * @return A newly created GFlatGenomeBase holding the crossed-over genome, upcast to GCandidateSolution
 */
std::shared_ptr<GCandidateSolution> GFlatGenomeBase::crossOverWith(GFlatGenomeBase const &cp) const {
    std::shared_ptr<GFlatGenomeBase> this_cp = this->clone<GFlatGenomeBase>();
 ...
 * @param cp A reference to the foreign genome (a GFlatGenomeBase); it is moved-from and cleared
 */
void GFlatGenomeBase::cannibalize(GFlatGenomeBase &cp) {
    if(cp.is_due_for_processing() || cp.has_errors()) {
        throw geneva_exception(
            g_error_streamer(DO_LOG, Gem::Common::timeAndPlace())
            << "In GFlatGenomeBase::cannibalize(): Error!" << '\n'
 ...
 */
double GFlatGenomeBase::getVarVal_d_(std::size_t idx) {
    std::vector<double> v;
 ...
 */
float GFlatGenomeBase::getVarVal_f_(std::size_t idx) {
    std::vector<float> v;
 ...
 */
std::int32_t GFlatGenomeBase::getVarVal_i_(std::size_t idx) {
 ...
 */
bool GFlatGenomeBase::getVarVal_b_(std::size_t idx) {
    std::vector<bool> v;
 ...
 */
void GFlatGenomeBase::toPropertyTree(pt::ptree &ptr, std::string const &base_name) const {
    ...
    ptr.put(base_name + ".type", std::string("GFlatGenomeBase"));
 ...
 */
std::string GFlatGenomeBase::toCSV(
 ...
 */
bool GFlatGenomeBase::modify_GUnitTests_() {
#ifdef GEM_TESTING
    bool result = false;
    ...
    return result;
#else  /* GEM_TESTING */
    Gem::Common::condnotset("GFlatGenomeBase::modify_GUnitTests", "GEM_TESTING");
    return false;
#endif /* GEM_TESTING */
}
 ...
 */
void GFlatGenomeBase::specificTestsNoFailureExpected_GUnitTests_() {
#ifdef GEM_TESTING
    {
        std::shared_ptr<GFlatGenomeBase> p_test = this->clone<GFlatGenomeBase>();
    ...
    Gem::Common::condnotset(
        "GFlatGenomeBase::specificTestsNoFailureExpected_GUnitTests",
        "GEM_TESTING"
    );
#endif /* GEM_TESTING */
}
 ...
 */
void GFlatGenomeBase::specificTestsFailuresExpected_GUnitTests_() {
#ifdef GEM_TESTING
    // no tests here yet
#else  /* GEM_TESTING */
    Gem::Common::condnotset(
        "GFlatGenomeBase::specificTestsFailuresExpected_GUnitTests",
        "GEM_TESTING"
    );
#endif /* GEM_TESTING */
}
