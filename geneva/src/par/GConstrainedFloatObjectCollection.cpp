...
BOOST_CLASS_EXPORT_IMPLEMENT(Gem::Geneva::Parameters::GConstrainedFloatObjectCollection) // NOLINT
...
GConstrainedFloatObjectCollection::GConstrainedFloatObjectCollection(
    ...
    std::shared_ptr<GConstrainedFloatObject> tmpl_ptr
)
  : GParameterTCollectionT<GConstrainedFloatObject>(n_cp, tmpl_ptr) { /* nothing */
}
 ...
 */
GParameterBase *GConstrainedFloatObjectCollection::clone_() const {
 ...
 */
void GConstrainedFloatObjectCollection::compare_(
    ...
    // Check that we are dealing with a GConstrainedFloatObjectCollection reference independent of this object and convert the pointer
    const GConstrainedFloatObjectCollection *p_load =
        Gem::Common::g_convert_and_compare<GParameterBase, GConstrainedFloatObjectCollection>(cp, this);

    GToken token("GConstrainedFloatObjectCollection", e);

    // Compare our parent data ...
    Gem::Common::compare_base_t<GParameterTCollectionT<GConstrainedFloatObject>>(
        *this,
        *p_load,
        token
    );
 ...
 */
std::string GConstrainedFloatObjectCollection::name_() const {
 ...
 *
 * @param cp A copy of another GConstrainedFloatObjectCollection object, camouflaged as a GParameterBase
 */
void GConstrainedFloatObjectCollection::load_(const GParameterBase *cp) {
 ...
 */
bool GConstrainedFloatObjectCollection::modify_GUnitTests_() {
#ifdef GEM_TESTING
    this->fillWithObjects_(10);

    // Call the parent class'es function
    GParameterTCollectionT<GConstrainedFloatObject>::modify_GUnitTests_();

    return true;

#else /* GEM_TESTING */ // If this function is called when GEM_TESTING isn't set, throw
    Gem::Common::condnotset("GConstrainedFloatObjectCollection::modify_GUnitTests", "GEM_TESTING");
    return false;
#endif                  /* GEM_TESTING */
}
 ...
 */
void GConstrainedFloatObjectCollection::fillWithObjects_(const std::size_t &n_added_objects) {
#ifdef GEM_TESTING
    // A random generator
    ...
    for(std::size_t i = 0; i < n_added_objects; i++) {
        ...
        std::shared_ptr<GConstrainedFloatObject> gcdo_ptr;

        CHECK_NOTHROW(
            gcdo_ptr =
                std::make_shared<GConstrainedFloatObject>(-100., 100.)
        ); // Boundaries in the range [-100., 100.[

        // Add the adaptor
        CHECK_NOTHROW(gcdo_ptr->addAdaptor(gfga_ptr));

        // Randomly initialize the GConstrainedFloatObject object, so it is unique
        CHECK_NOTHROW(gcdo_ptr->randomInit(activityMode::ALLPARAMETERS, gr));

        // Add the object to the collection
        CHECK_NOTHROW(this->push_back(gcdo_ptr));
    }

    //---------------------------------------------------------------------------

#else /* GEM_TESTING */ // If this function is called when GEM_TESTING isn't set, throw
    Gem::Common::condnotset("GConstrainedFloatObjectCollection::fillWithObjects", "GEM_TESTING");
#endif                  /* GEM_TESTING */
}
 ...
 */
void GConstrainedFloatObjectCollection::specificTestsNoFailureExpected_GUnitTests_() {
#ifdef GEM_TESTING
    // Some settings
    ...
    { // Call the parent class'es function
        std::shared_ptr<GConstrainedFloatObjectCollection> p_test =
            this->clone<GConstrainedFloatObjectCollection>();
        ...
        p_test->GParameterTCollectionT<
            GConstrainedFloatObject>::specificTestsNoFailureExpected_GUnitTests_();
    }

    // --------------------------------------------------------------------------

    { // Test the GParameterTCollectionT<T>::adapt implementation
        std::shared_ptr<GConstrainedFloatObjectCollection> p_test1 =
            this->clone<GConstrainedFloatObjectCollection>();
        std::shared_ptr<GConstrainedFloatObjectCollection> p_test2 =
            this->clone<GConstrainedFloatObjectCollection>();
    ...
    Gem::Common::condnotset(
        "GConstrainedFloatObjectCollection::specificTestsFailuresExpected_GUnitTests",
        "GEM_TESTING"
    );
#endif                  /* GEM_TESTING */
}
 ...
 */
void GConstrainedFloatObjectCollection::specificTestsFailuresExpected_GUnitTests_() {
#ifdef GEM_TESTING
    // Some settings
    constexpr std::size_t n_added_objects = 10;

    // Call the parent class'es function
    GParameterTCollectionT<GConstrainedFloatObject>::specificTestsFailuresExpected_GUnitTests_();
