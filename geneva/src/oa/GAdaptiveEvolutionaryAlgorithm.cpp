...
BOOST_CLASS_EXPORT_IMPLEMENT(Gem::Geneva::OptimizationAlgorithms::GAdaptiveEvolutionaryAlgorithm) // NOLINT

/******************************************************************************/

namespace Gem::Geneva::OptimizationAlgorithms {

using GType = GAdaptiveEvolutionaryAlgorithm;
using TraitsType = GAdaptiveEvolutionaryAlgorithm_PersonalityTraits;

/******************************************************************************/
/**
 * @brief The default constructor. Sets a valid default population size.
 */
GAdaptiveEvolutionaryAlgorithm::GAdaptiveEvolutionaryAlgorithm() {
...
void GType::compare_(
    ...
    GToken token("GAdaptiveEvolutionaryAlgorithm", e);
...
void GType::updateGlobalBestsPQ_(
    ...
    if(this->empty()) {
        throw geneva_exception(
            ...
            << "In GAdaptiveEvolutionaryAlgorithm::updateGlobalBestsPQ_() :" << '\n'
...
void GType::updateIterationBestsPQ_(
    ...
    if(this->empty()) {
        throw geneva_exception(
            g_error_streamer(DO_LOG, Gem::Common::timeAndPlace())
            << "GAdaptiveEvolutionaryAlgorithm::updateIterationBestsPQ_() :" << '\n'
            << "Tried to retrieve the best individuals even though the population is empty." << '\n'
        );
    }
#endif /* DEBUG */
...
void GType::populationSanityChecks_() const {
    if(this->n_parents_ == 0) {
        throw geneva_exception(
            g_error_streamer(DO_LOG, Gem::Common::timeAndPlace())
            << "In GAdaptiveEvolutionaryAlgorithm::populationSanityChecks(): Error!" << '\n'
    ...
    if(
        ...
        error << "In GAdaptiveEvolutionaryAlgorithm::populationSanityChecks() :" << '\n'
...
void GType::runFitnessCalculation_() {
    ...
    for(std::size_t i = this->getNParents(); i < this->size(); i++) {
        if(not this->at(i)->individual().is_due_for_processing()) {
            throw geneva_exception(
                g_error_streamer(DO_LOG, Gem::Common::timeAndPlace())
                << "In GAdaptiveEvolutionaryAlgorithm::runFitnessCalculation(): Error!" << '\n'
...
void GType::selectBest_() {
    ...
    if((this->size() - this->n_parents_) < this->default_n_children_) {
        throw geneva_exception(
            g_error_streamer(DO_LOG, Gem::Common::timeAndPlace())
            << "In GAdaptiveEvolutionaryAlgorithm::select():" << '\n'
    ...
    if(this->size() < this->getDefaultPopulationSize()) {
        throw geneva_exception(
            g_error_streamer(DO_LOG, Gem::Common::timeAndPlace())
            << "In GAdaptiveEvolutionaryAlgorithm::selectBest(): Error!" << '\n'
...
void GType::sortMuPlusNuParetoMode() {
    ...
    if(not(*it)->individual().hasMultipleFitnessCriteria()) {
        ...
        if(not warned.exchange(true)) {
            glogger << "In GAdaptiveEvolutionaryAlgorithm::sortMuPlusNuParetoMode(): Warning!" << '\n'
...
void GType::sortMuCommaNuParetoMode() {
    ...
    if(not(*it)->individual().hasMultipleFitnessCriteria()) {
        static std::atomic<bool> warned{false};
        if(not warned.exchange(true)) {
            glogger << "In GAdaptiveEvolutionaryAlgorithm::sortMuCommaNuParetoMode(): Warning!" << '\n'
                    << "A PARETO sorting mode was selected, but the individuals expose only a single" << '\n'
                    << "fitness criterion. Pareto selection therefore degenerates to single-objective" << '\n'
                    << "MUCOMMANU selection." << '\n'
                    << GWARNING;
...
bool GType::aDominatesB(
    ...
    if(n_criteria_x != n_criteria_y) {
        throw geneva_exception(
            g_error_streamer(DO_LOG, Gem::Common::timeAndPlace())
            << "In GAdaptiveEvolutionaryAlgorithm::aDominatesB(): Error!" << '\n'
...
#else /* GEM_TESTING */
    Gem::Common::condnotset("GAdaptiveEvolutionaryAlgorithm::modify_GUnitTests", "GEM_TESTING");
    return false;
#endif                  /* GEM_TESTING */
}
...
#else /* GEM_TESTING */
    Gem::Common::condnotset("GAdaptiveEvolutionaryAlgorithm::fillWithObjects", "GEM_TESTING");
#endif                  /* GEM_TESTING */
}
...
void GType::specificTestsNoFailureExpected_GUnitTests_() {
    ...
    Gem::Common::condnotset(
        "GAdaptiveEvolutionaryAlgorithm::specificTestsNoFailureExpected_GUnitTests",
        "GEM_TESTING"
    );
#endif                  /* GEM_TESTING */
}
...
#else  /* GEM_TESTING */
    Gem::Common::condnotset(
        "GAdaptiveEvolutionaryAlgorithm::specificTestsFailuresExpected_GUnitTests",
        "GEM_TESTING"
    );
#endif /* GEM_TESTING */
}
