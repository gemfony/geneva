...
double GImageIndividual::fitnessCalculation() {
    // EXPERIMENTAL PORT: unlike the original example 15 (whose host fitnessCalculation threw, forcing
    // GPU evaluation), this computes the fitness on the CPU via the SAME render+score the GPU kernel
    // uses (Gem::Geneva::MonaLisa::score, shared in GMonaLisaProblem.hpp). This makes the individual
