...
constexpr float RSF_FACTOR = 0.04f; ///< rational-saturation-function factor (matches example 15)

/** @brief The target image, loaded once from a PNG: width/height and W*H*3 RGB floats in [0,1]. */
struct Target {
    int width = 0;
    int height = 0;
...
inline void calculateCorners(const float *tri, int width, int height, float *out6) {
    const float cx = tri[0] * width;
    const float cy = tri[1] * height;
    const float radius = tri[2];
    const float scale = static_cast<float>(width < height ? width : height);
...
inline bool pointInTriangle(float px, float py, const float *c) {
    const float x1 = c[0], y1 = c[1], x2 = c[2], y2 = c[3], x3 = c[4], y3 = c[5];
    const float minx = std::min(x1, std::min(x2, x3));
    const float maxx = std::max(x1, std::max(x2, x3));
    const float miny = std::min(y1, std::min(y2, y3));
    const float maxy = std::max(y1, std::max(y2, y3));
 ...
 */
inline float score(const float *params, int dim, int W, int H, const float *targetRGB,
                   std::vector<float> &cornerScratch) {
    const int NT = (dim - 3) / 10;
    const float bgR = params[10 * NT + 0];
    const float bgG = params[10 * NT + 1];
    const float bgB = params[10 * NT + 2];
    ...
    for(int y = 0; y < H; ++y) {
        for(int x = 0; x < W; ++x) {
            ...
            const float dr = targetRGB[idx] - r;
            const float dg = targetRGB[idx + 1] - g;
            const float db = targetRGB[idx + 2] - b;
 ...
 */
inline void renderToRGB(const float *params, int dim, int W, int H,
    ...
    std::vector<float> corners(static_cast<std::size_t>(NT) * 6);
    for(int t = 0; t < NT; ++t) {
...
inline float scoreAgainstTarget(const float *params, int dim) {
