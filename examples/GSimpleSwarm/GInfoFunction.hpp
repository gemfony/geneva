 ...
 */
const boost::uint16_t DEFAULTXDIM=800;

/**
 * The default dimension of the canvas in y-direction
 */
const boost::uint16_t DEFAULTYDIM=1200;
...
public:
	 ...
	 */
	optimizationMonitor(const std::size_t nInfoIndividuals, const Gem::Geneva::demoFunction& df, std::ostream& summary)
	...
	void informationFunction(const infoMode& im, GSwarm * const gs){
		...
		case Gem::Geneva::INFOPROCESSING:
		{
			...
			boost::uint32_t generation = gs->getIteration();

			summary_ << "  generation.push_back(" << generation << ");" << std::endl;

			for(std::size_t p=0; p<nInfoIndividuals_; p++) {
				// Get access to the inidividual
				boost::shared_ptr<GStartIndividual> gdii_ptr = gs->individual_cast<GStartIndividual>(p);
	 ...
	 */
	void setDims(const boost::uint16_t& xDim, const boost::uint16t_t yDim) {
		xDim_ = xDim;
		yDim_ = yDim;
	}
