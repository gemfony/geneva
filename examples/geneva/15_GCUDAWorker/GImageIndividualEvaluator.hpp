...
__device__ inline void cuda_calculateCorners(
    const CircleTriangle &tri,
    const int &width,
    const int &height,
    float &outX1,
    float &outY1,
...
__device__ bool cuda_pointInTriangle(
    const float px,
    const float py,
    const float x1,
    const float y1,
...
__device__ inline void cuda_alphaBlend(
    float &bgR,
    float &bgG,
    float &bgB,
    const float fgR,
...
__global__ void
cuda_transformTriangleData(const CircleTriangle *, float *, const int, const int, const int);

template <int ExtractionMode>
__global__ void cuda_renderAndCompareKernel(
...
private:
    static void checkCuda(cudaError_t err, const char *msg) {
        if(err != cudaSuccess) {
            fprintf(
                stderr,
                "GImageIndividualEvaluator::checkCuda: CUDA Error! %s (%s)\n",
                msg,
    ...
    static bool cpu_pointInTriangle(
        float px,
        float py,
        float x1,
        float y1,
    ...
    static void
    cpu_alphaBlend(float &, float &, float &, const float, const float, const float, const float);
    ...
    float *d_perPixel_evaluation_{
        nullptr
    }; ///< Holds the evaluations for each color channel of each pixel
    float *d_bgcolor_{
        nullptr
    }; ///< Holds the current background color, to be transferred to the device
    Geneva::CircleTriangle *d_triangles_{
        nullptr
    }; ///< Holds the "raw" triangles described by the individual
