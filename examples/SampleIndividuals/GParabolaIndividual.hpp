...
public:
	...
	GParabolaIndividual(std::size_t sz, double min, double max){
		...
		// knows how to be mutated. We want a sigma depenent on max, sigma-adaption of 0.001 and
		// a minimum sigma of 0.002.
		boost::shared_ptr<GDoubleGaussAdaptor> gdga(new GDoubleGaussAdaptor(0.1*fabs(max),0.001,0.002,"gauss_mutation"));
		gdc->addAdaptor(gdga);
