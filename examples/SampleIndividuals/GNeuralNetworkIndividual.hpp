...
public:
		 ...
		 transferMode_(SIGMOID)
	{
		...
		for(layerIterator=architecture_.begin(); layerIterator!=architecture_.end(); ++layerIterator){
			if(*layerIterator){ // Add the next network layer to this class, if possible
				...
				// knows how to be mutated. We want a sigma of 0.1, sigma-adaption of 0.01 and
				// a minimum sigma of 0.001.
				boost::shared_ptr<GDoubleGaussAdaptor> gdga(new GDoubleGaussAdaptor(0.1,0.01,0.001,"gauss_mutation"));
				gdc->addAdaptor(gdga);
	...
	std::string writeTrainedNetwork(std::string fileName) {
		...
		network << "      const std::size_t nWeights = " << nWeights << ";" << std::endl
		...
		for(std::size_t i=0; i<architecture_.size(); i++) {
			...
			for(std::size_t j=0; j<currentLayer.size(); j++) {
				network << "        " << currentLayer[j];

				if(i==(architecture_.size()-1) && j==(currentLayer.size()-1)) network << std::endl;
				else network << "," << std::endl;
			}
		}
		...
		network << "      };" << std::endl
				...
				<< "        currentLayer += weightOffset[layerCounter]; // shift the start of the array" << std::endl
				<< std::endl
				...
				<< "            nodeResult += prevResults[prevNodeCounter]*currentLayer[nodeCounter*(nPrevLayerNodes+1)+prevNodeCounter];" << std::endl
				<< "          }" << std::endl
		...
		if(fileName != "") {
			...
			if(fstr) {
				...
				std::ostringstream error;
				error << "In GNeuralNetworkIndividual::writeTrainedNetwork(std::string) : Error writing output file!" << std::endl;
