...
double solver::fitnessAckley(const std::vector<double> &vec) const {
    ...
    for(std::size_t i = 0; i < (vec.size() - 1); i++) {
        result +=
            (exp(-0.2) * sqrt(pow(vec.at(i), 2.) + pow(vec.at(i + 1), 2.)) +
             3. * (cos(2. * vec.at(i)) + sin(2. * vec.at(i + 1))));
    }
    return result;
}
...
optimizerBase::optimizerBase(
  ...
  , solver_(s)
  , maxIterations_(maxIterations) { /* nothing */
}
 ...
 * The destructor
 */
optimizerBase::~optimizerBase() { /* nothing */
}
 ...
 */
void optimizerBase::init() { /* nothing */ };

/******************************************************************************/
/**
 * Overload this function in derived classes, if initialization work is required
 */
void optimizerBase::finalize() { /* nothing */ };
 ...
 */
void print(const std::vector<double> &vec, const std::string &envelope) {
