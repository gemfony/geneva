...
| `courtier` | Consumer-based parallelization framework. A process uses ONE consumer, held in a process-global `GConsumerRegistry`: `GSerialConsumerT`, `GStdThreadConsumerT`, `GAsioConsumerT`, `GWebsocketConsumerT`, or `GMPIConsumerT`. An algorithm submits a batch straight to that consumer (`GBaseConsumerT::processBatch`), which evaluates and reconciles it against the submission policy |
