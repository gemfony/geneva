...
bool GSerialPS::operator==(const GSerialPS& cp) const {
   ...
   return !checkRelationshipWith(cp, CE_EQUALITY, 0.,"GSerialPS::operator==","cp", CE_SILENT);
}
...
bool GSerialPS::operator!=(const GSerialPS& cp) const {
   ...
   return !checkRelationshipWith(cp, CE_INEQUALITY, 0.,"GSerialPS::operator!=","cp", CE_SILENT);
}
...
void GSerialPS::compare(
   ...
   try {
      ...
      g.add("g_expectation_violation caught by GSerialPS");
      throw g;
   }
}
