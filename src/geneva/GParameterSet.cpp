...
namespace Gem::Geneva
{
    ...
    std::shared_ptr<GParameterSet> GParameterSet::crossOverWith(std::shared_ptr<GParameterSet> const& cp) const
    {
        ...
        if (this_double_cnt.size() != cp_double_cnt.size())
        {
            throw gemfony_exception(
                ...
                << "Got invalid sizes (double): " << this_double_cnt.size() << " / " << cp_double_cnt.size() <<
                std::endl
            );
        }
        if (this_float_cnt.size() != cp_float_cnt.size())
        {
            throw gemfony_exception(
                g_error_streamer(DO_LOG, time_and_place)
                << "In GParameterSet::perItemCrossOver(): Error!" << std::endl
                << "Got invalid sizes (float): " << this_float_cnt.size() << " / " << cp_float_cnt.size() <<
                std::endl
            );
        }
        ...
        if (this_int_cnt.size() != cp_int_cnt.size())
        {
            throw gemfony_exception(
                g_error_streamer(DO_LOG, time_and_place)
                << "In GParameterSet::perItemCrossOver(): Error!" << std::endl
                << "Got invalid sizes (std::int32_t): " << this_int_cnt.size() << " / " << cp_int_cnt.size() <<
                std::endl
            );
        }
#endif /* DEBUG */
    ...
    )
    {
        ...
        if (id >= this->getNStoredResults())
        {
            throw gemfony_exception(
                ...
                << "Invalid position in vector: " << id << " (expected min 0 and max " << this->getNStoredResults()
                - 1 << ")" << std::endl
            );
        }
#endif /* DEBUG */
    ...
    )
    {
        ...
        gpb.registerFileParameter<bool, double>(
            ...
            << "Indicates whether random crashes should occur for debugging purposes" << std::endl <<
    ...
    double GParameterSet::weighedSquaredSumCombiner(std::vector<double> const& weights) const
    {
        if (this->getNStoredResults() != weights.size())
        {
            throw gemfony_exception(
                ...
                << "Sizes of transformedCurrentFitnessVec_ and the weights vector don't match: " << this->
                getNStoredResults() <<
    ...
    void GParameterSet::specificTestsNoFailureExpected_GUnitTests_()
    {
...

        {
            ...
            while (true)
            {
                ...
                BOOST_CHECK_MESSAGE(
                    ...
                    << "p_test->getBestKnownPrimaryFitness() = " << Gem::Common::g_to_string(p_test->
...

        {
            ...
            for (std::size_t i = 0; i < FPLOOPCOUNT; i++)
            {
                    ...
                    std::shared_ptr<GConstrainedDoubleObject>(
                        new GConstrainedDoubleObject(
                            uniform_real_distribution(
                                m_gr, std::uniform_real_distribution<double>::param_type(
                ...
                p_test_0->push_back(
                    std::shared_ptr<GDoubleCollection>(
...

            {
                ...
                BOOST_CHECK_NO_THROW(
                    p_test->multiplyByRandom<double>(FPMULTIPLYBYRANDMIN, FPMULTIPLYBYRANDMAX, activityMode::
                        ALLPARAMETERS));
                ...
                for (std::size_t i = 0; i < FPLOOPCOUNT; i++)
                {
                    ...
                    counter++;
                    BOOST_CHECK(
                        p_test->at<GDoubleObject>(counter)->value() != p_test_0->at<GDoubleObject>(counter)->value(
                        ));
                    counter++;
                    std::shared_ptr<GDoubleCollection> p_gdc;
...

        {
            ...
            for (std::size_t i = 0; i < FPLOOPCOUNT; i++)
            {
                ...
                std::shared_ptr<GDoubleObject> gdo_ptr = std::shared_ptr<GDoubleObject>(
                    new GDoubleObject(
                        uniform_real_distribution(
                            m_gr, std::uniform_real_distribution<double>::param_type(MINGDOUBLE, MAXGDOUBLE))
                    )
                );
