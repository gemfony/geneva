...
void GBrokerEA::runFitnessCalculation() {
   ...
   for(std::size_t i=this->getNParents(); i<this->size(); i++) {
	...
	//--------------------------------------------------------------------------------
	// Now fix the population -- it may be smaller than its nominal size. Other parts
	// of this class expect that there is a defined number of individuals in the
	// population. However, not all individuals might have returned.
...
void GBrokerEA::fixAfterJobSubmission() {
	...
	if(data.empty()) {
		glogger
			<< "In GBrokerEA::fixAfterJobSubmission(): Error!" << std::endl
		...
		if(data.size() <= this->getNParents()) {
			glogger
				<< "In GBrokerEA::fixAfterJobSubmission(): Warning!" << std::endl
 ...
 */
void GBrokerEA::setNThreads(std::uint16_t nThreads) {
	if (nThreads == 0) {
		m_n_threads = boost::numeric_cast<std::uint16_t>(Gem::Common::getNHardwareThreads(DEFAULTNSTDTHREADS));
	}
	else {
 ...
 */
std::uint16_t GBrokerEA::getNThreads() const {
	return m_n_threads;
}
