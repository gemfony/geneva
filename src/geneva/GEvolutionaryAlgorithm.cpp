...
GEvolutionaryAlgorithm::GEvolutionaryAlgorithm()
	...
	, parallelizationMode_(DEFAULTPARALLELIZATIONMODE)
{
...
GEvolutionaryAlgorithm::GEvolutionaryAlgorithm(const GEvolutionaryAlgorithm& cp)
	...
	, parallelizationMode_(cp.parallelizationMode_)
{
...
void GEvolutionaryAlgorithm::load_(const GObject * cp)
{
	...
	parallelizationMode_ = p_load->parallelizationMode_;
}
		...
		const bool& withMessages) const
{
	...
	deviations.push_back(checkExpectation(withMessages, "GEvolutionaryAlgorithm", parallelizationMode_, p_load->parallelizationMode_, "parallelizationMode_", "p_load->parallelizationMode_", e , limit));
 ...
 */
void GEvolutionaryAlgorithm::setLogOldParents(bool logOldParents) {
...
/************************************************************************************************************/
/**
 * Allows to specify the parallelization mode to be used. Allowed values are 0 (serial), 1 (multi-threaded)
 * or 2 (broker)
 *
 * @param parallelizationMode The desired parallelization mode
 */
void GEvolutionaryAlgorithm::setParallelizationMode(parMode parallelizationMode) {
	parallelizationMode_ = parallelizationMode;
}

/************************************************************************************************************/
/**
 * Allows to retrieve the current parallelization mode
 */
parMode GEvolutionaryAlgorithm::getParallelizationMode() const {
	return parallelizationMode_;
}
 ...
 */
void GEvolutionaryAlgorithm::setDefaultPopulationSize(std::size_t popSize, std::size_t nParents) {
 ...
 */
boost::shared_ptr<GIndividual> GEvolutionaryAlgorithm::getBestIndividual() {
#ifdef DEBUG
		if(data.empty()) {
			raiseException(
					"In GEvolutionaryAlgorithm::getBestIndividual() :" << std::endl
...
void GEvolutionaryAlgorithm::addConfigurationOptions (
	...
	if(showOrigin) comment1 += " [GEvolutionaryAlgorithm]";
	...
	gpb.registerFileParameter<std::size_t, std::size_t>(
		...
		, DEFAULTEANPARENTS
		, boost::bind(
			&GEvolutionaryAlgorithm::setDefaultPopulationSize
			, this
			, _1
			, _2
		  )
	...
	gpb.registerFileParameter<sortingMode>(
		...
		, boost::bind(
			&GEvolutionaryAlgorithm::setSortingScheme
			, this
			, _1
		  )
		, Gem::Common::VAR_IS_ESSENTIAL // Alternative: VAR_IS_SECONDARY
	...
	gpb.registerFileParameter<std::size_t, std::size_t>(
		...
		, boost::bind(
			&GEvolutionaryAlgorithm::setPopulationGrowth
			, this
			, _1
			, _2
		  )
	...
	gpb.registerFileParameter<double>(
		...
		, boost::bind(
			&GEvolutionaryAlgorithm::setTDegradationStrength
			, this
			, _1
		)
		, Gem::Common::VAR_IS_ESSENTIAL // Alternative: VAR_IS_SECONDARY
	...
	gpb.registerFileParameter<bool>(
		...
		, boost::bind(
			&GEvolutionaryAlgorithm::setLogOldParents
			, this
			, _1
		  )
		, Gem::Common::VAR_IS_SECONDARY // Alternative: VAR_IS_ESSENTIAL
	...
	comment = ""; // Reset the comment string
	comment += "Allows to set the parallelization mode used by this;";
	comment += "optimization algorithm. 0 means \"serial execution\",;";
	comment += "1 means \"multi-threaded execution\", 2 means \"brokerage\";";
	if(showOrigin) comment += "[GEvolutionaryAlgorithm]";
	gpb.registerFileParameter<bool>(
		"parallelizationMode" // The name of the variable
		, DEFAULTPARALLELIZATIONMODE // The default value
		, boost::bind(
			&GEvolutionaryAlgorithm::setParallelizationMode
			, this
			, _1
		  )
		, Gem::Common::VAR_IS_ESSENTIAL // Alternative: VAR_IS_SECONDARY
 ...
 */
void GEvolutionaryAlgorithm::setTDegradationStrength(double alpha) {
	if(alpha <=0.) {
		raiseException(
 ...
 */
void GEvolutionaryAlgorithm::setT0(double t0) {
	if(t0 <=0.) {
		raiseException(
 ...
 */
void GEvolutionaryAlgorithm::setRecombinationMethod(recoScheme recombinationMethod) {
