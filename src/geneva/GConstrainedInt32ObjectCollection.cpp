...
void GConstrainedInt32ObjectCollection::compare_(
    ...
    Gem::Common::compare_base_t<GParameterTCollectionT<GConstrainedInt32Object>>(
        *this,
        *p_load,
        token
    );
...
void GConstrainedInt32ObjectCollection::fillWithObjects_(const std::size_t &n_added_objects) {
    ...
    for(std::size_t i = 0; i < n_added_objects; i++) {
        ...
        // Add the adaptor
        CHECK_NOTHROW(gcio_ptr->addAdaptor(giga_ptr));

        // Randomly initialize the GConstrainedInt32Object object, so it is unique
        CHECK_NOTHROW(gcio_ptr->randomInit(activityMode::ALLPARAMETERS, gr));

        // Add the object to the collection
        CHECK_NOTHROW(this->push_back(gcio_ptr));
    }
...
void GConstrainedInt32ObjectCollection::specificTestsNoFailureExpected_GUnitTests_() {
    ...
    { // Call the parent class'es function
        std::shared_ptr<GConstrainedInt32ObjectCollection> p_test =
        ...
        p_test->GParameterTCollectionT<
            GConstrainedInt32Object>::specificTestsNoFailureExpected_GUnitTests_();
    }

    // --------------------------------------------------------------------------

    { // Test that the fp-family of functions has no effect on this object (and contained objects)
        std::shared_ptr<GConstrainedInt32ObjectCollection> p_test1 =
            this->clone<GConstrainedInt32ObjectCollection>();
        std::shared_ptr<GConstrainedInt32ObjectCollection> p_test2 =
            this->clone<GConstrainedInt32ObjectCollection>();
    ...
    Gem::Common::condnotset(
        "GConstrainedInt32ObjectCollection::specificTestsNoFailureExpected_GUnitTests",
        "GEM_TESTING"
    );
#endif                  /* GEM_TESTING */
}
...
#else /* GEM_TESTING */ // If this function is called when GEM_TESTING isn't set, throw
    Gem::Common::condnotset(
        "GConstrainedInt32ObjectCollection::specificTestsFailuresExpected_GUnitTests",
        "GEM_TESTING"
    );
#endif                  /* GEM_TESTING */
}
