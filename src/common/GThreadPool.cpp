    ...
    : m_nThreads(nThreads > 0 ? nThreads : DEFAULTNHARDWARETHREADS)
{
    if (0 == nThreads)
    {
        glogger
            ...
            << "User requested nThreads == 0. nThreads was reset to the default " << DEFAULTNHARDWARETHREADS <<
            std::endl
            << GWARNING;
    }
}
 ...
 */
void
GThreadPool::setNThreads(unsigned int nThreads)
{
