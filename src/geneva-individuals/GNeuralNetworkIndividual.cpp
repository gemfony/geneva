...
void networkData::loadFromDisk(const std::string &networkDataFile) {
    ...
    if(not trDat) {
        ...
        throw geneva_exception(
            g_error_streamer(DO_LOG, time_and_place) << error.str() << std::endl
        );
    }
...
void networkData::toROOT(const std::string &outputFile, const double &min, const double &max) {
    ...
    if(getNInputNodes() != 2 || getNOutputNodes() != 1) {
        glogger << "In networkData::toRoot(): Warning!" << std::endl
                << "Got inappropriate number of input and/or output nodes: " << getNInputNodes()
                << "/" << getNOutputNodes() << std::endl
    ...
    of << "{" << std::endl
       ...
       << "], yarr2[" << arraySize_ << "];" << std::endl
       << std::endl
       << "  // Filling the data sets" << std::endl;

    for(std::size_t i = 0; i < arraySize_; i++) {
        if(data_[i]->Output[0] < 0.5) {
            of << "  xarr1[" << entries1 << "] = " << data_[i]->Input[0] << ";" << std::endl
               << "  yarr1[" << entries1 << "] = " << data_[i]->Input[1] << ";" << std::endl;
            entries1++;
        }
        else {
            of << "  xarr2[" << entries2 << "] = " << data_[i]->Input[0] << ";" << std::endl
