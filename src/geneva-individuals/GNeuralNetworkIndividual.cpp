...
networkData::networkData(const std::size_t &arraySize)
	: Gem::Common::GStdSimpleVectorInterfaceT<std::size_t>(), arraySize_(arraySize),
...
void networkData::toROOT(
	...
	if (getNInputNodes() != 2 || getNOutputNodes() != 1) {
		...
		<< "Got inappropriate number of input and/or output nodes: " << getNInputNodes() << "/" << getNOutputNodes() <<
		std::endl
