...
BOOST_CLASS_EXPORT(GenEvA::GTransferPopulation)
 ...
 */
GTransferPopulation::GTransferPopulation(void) :
	GBasePopulation() {
 ...
 *
 * \param cp A copy of another GTransferPopulation object
 */
GTransferPopulation::GTransferPopulation(const GTransferPopulation& cp) :
	GBasePopulation(cp) {
 ...
 */
GTransferPopulation::~GTransferPopulation()
{ /* nothing */}

/******************************************************************************/
/**
 * A standard assignment operator for GTransferPopulation objects,
 *
 * \param cp A copy of another GTransferPopulation object
 * \return A constant reference to this object
 */
const GTransferPopulation& GTransferPopulation::operator=(const GTransferPopulation& cp) {
	GTransferPopulation::load(&cp);
	return *this;
}
 ...
 * A standard assignment operator for GTransferPopulation objects,
 *
 * \param cp A copy of another GTransferPopulation object, camouflaged as a GObject
 * \return A constant reference to this object
 */
const GObject& GTransferPopulation::operator=(const GObject& cp) {
	GTransferPopulation::load(&cp);
	return *this;
}
 ...
 */
void GTransferPopulation::reset(void) {
	waitFactor_=DEFAULTWAITFACTOR;
...
/******************************************************************************/
/**
 * Loads the data of another GTransferPopulation object, camouflaged as a
 * pointer to a GObject
 *
 * \param cp A pointer to another GTransferPopulation object, camouflaged as a GObject
 */
void GTransferPopulation::load(const GObject * cp) {
	...
	if(static_cast<const GTransferPopulation *>(cp) == this) {
		GException ge;
		ge << "In GTransferPopulation::load(): Error!" << endl
	...
	const GTransferPopulation * gtp = dynamic_cast<const GTransferPopulation *>(cp);
	if(!gtp) {
		GException ge;
		ge << "In GTransferPopulation::load() : Conversion Error!" << endl
		<< raiseException;
	}
 ...
 */
GObject *GTransferPopulation::clone(void) {
	return new GTransferPopulation(*this);
}
 ...
 */
void GTransferPopulation::setWaitFactor(uint16_t waitFactor) {
 ...
 */
uint16_t GTransferPopulation::getWaitFactor(void) const {
	return waitFactor_;
}
 ...
 */
void GTransferPopulation::setFirstTimeOut(int32_t d, int32_t h, int32_t m, int32_t s) {
 ...
 */
int32_t GTransferPopulation::getFirstTimeOut(void) const {
 ...
 */
void GTransferPopulation::setLoopTime(uint16_t loopSec, uint16_t loopMSec) {
	// Only allow "real" values
	if(loopSec==0 && loopMSec==0) {
		GException ge;
		ge << "In GTransferPopulation::setLoopTime() : Error!" << endl
 ...
 */
uint16_t GTransferPopulation::getLoopSec(void) const {
	return loopSec_;
}
 ...
 */
uint16_t GTransferPopulation::getLoopMSec(void) const {
	return loopMSec_;
}

/******************************************************************************/
/**
 * This function emits information about the state of this population.
 *
 ...
 */
string GTransferPopulation::assembleReport(uint16_t indention) const
{
	ostringstream oss;
	oss << ws(indention) << "GTransferPopulation: " << this << endl
	<< ws(indention) << "firstTimeOut_ = " << firstTimeOut_ << endl
	<< ws(indention) << "loopMSec_ = " << loopMSec_ << endl
	<< ws(indention) << "loopSec_ = " << loopSec_ << endl
	<< ws(indention) << "waitFactor_ = " << waitFactor_ << endl
 ...
 */
void GTransferPopulation::optimize(void) {
	string id = GBasePopulation::getId();
	...
	if(!CurrentGBiBufferPtr_) {
		GException ge;
		ge << "In GTransferPopulation::optimize(void) : Error!" << endl
 ...
 * the GTransferPopulation::select() function.
 */
void GTransferPopulation::mutateChildren(void) {
	uint16_t np = getNParents(), nc = getNChildren();
	uint32_t generation = GBasePopulation::getGeneration();
	vector<boost::shared_ptr<GMember> >::reverse_iterator rit;
	vector<boost::shared_ptr<GMember> >::iterator it;
	string id = GBasePopulation::getId();
	uint16_t nParentsSentAway = 0;
	...
	if(generation==0 && getSortingScheme()==MUPLUSNU) {
		...
		vector<boost::shared_ptr<GMember> > tempParents;
	...
	bool ok = false;
	uint16_t nReceivedCurrent = 0;
	uint16_t nReceivedOlder = 0;
	...
	while(true) {
		...
		if(getFirstTimeOut() && ((firstTimeMissed-startTime)> firstTimeOut_)) {
			GException ge;
			ge << "In GTransferPopulation::mutateChildren(void) : Error!" << endl
	...
	while(true) {
		...
		if(ok) {
			...
			// fuer MUPLUSNU ...
			uint16_t defaultChildren = getDefaultChildren();
			if(((generation==0) && (getSortingScheme()==MUPLUSNU))?
		...
		if(getWaitFactor() && (totalElapsed> totalElapsedFirst*getWaitFactor())) {
			GLogStreamer gls;
			gls << "In GTransferPopulation::mutateChildren(void):" << this << " : " << endl
 ...
 */
void GTransferPopulation::select(void) {
	uint16_t sz = this->size();
	uint32_t generation = this->getGeneration();
	uint16_t npar, nParents=this->getNParents();
	...
	if(sz==0) {
		...
		GException ge;
		ge << "In GTransferPopulation::select(void) : Error in population " << this << endl
	...
	if(generation==0 && sortingScheme==MUPLUSNU) {
		...
		if(npar < nParents) {
			...
			if(npar==0) {
				gls << "In GTransferPopulation::select(void): Warning in population " << this << endl
			...
			uint16_t missing = nParents - npar;

			// Let the user know
			gls << "In GTransferPopulation::select(void): Adding " << missing << " missing" << endl
			...
			for(uint16_t i=0; i<missing; i++) {
				...
				if(!newParent) {
					GException ge;
					ge << "In GTransferPopulation::select(void): Conversion error "
	...
	uint16_t currentChildren = this->size() - nParents;
	uint16_t defaultChildren = getDefaultChildren();

	if(currentChildren < defaultChildren) {
		uint16_t missingChildren = defaultChildren - currentChildren;

		// Let the user know
		gls << "In GTransferPopulation::select(void): Adding " << missingChildren << " missing" << endl
		<< "children to the population " << this << endl
		<< logLevel(UNCRITICAL);

		// Add copies of last available member
		for(uint16_t i = 0; i < missingChildren; i++) {
			// Doing this with an iterator would be more difficult
			GMember *gm = dynamic_cast<GMember *>(this->back()->clone());
			if(!gm) {
				GException ge;
				ge << "In GTransferPopulation::select(): Conversion error (2)!" << endl
				<< raiseException;
			}
